...
const courseSchema = z.object({ title: z.string().min(2).max(140), description: z.string().max(3000).optional(), price: z.coerce.number().min(0).optional(), status: z.enum(['draft', 'published']).optional(), lessons: z.array(z.object({ title: z.string().min(2).max(180), content: z.string().max(20000).optional(), duration: z.coerce.number().min(1).max(600).optional(), isPreview: z.boolean().optional() })).max(100).optional() });
